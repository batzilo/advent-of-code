...
bool dpc_double_capacity(struct dynamic_packed_container* dpcp) {
  ...
  if (new_size == 0) {
    new_size = dpcp->element_size_bytes;
  }
  DPC_LOG_INFO(
      "resizing dynamic packed container, from size %lu bytes to size %lu "
      "bytes ...",
      old_size, new_size);
...
bool dpc_erase(struct dynamic_packed_container* dpcp) {
  ...
  void* elem_ptr =
      (char*)dpcp->data + dpcp->element_count * dpcp->element_size_bytes;
